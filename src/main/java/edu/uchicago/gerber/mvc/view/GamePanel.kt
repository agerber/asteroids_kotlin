...
class GamePanel(dim: Dimension) : Panel() {
    ...
    private fun drawFrame(g: Graphics) {
        g.color = Color.white
        g.font = panelFont
        g.drawString("KOTLIN :  " + CommandCenter.frame, fontWidth, Game.DIM.height - (fontHeight + 22))
    ...
    private fun drawFalconStatus(g: Graphics){
        ...
        g.drawString(levelText, 20, 30) //upper-left corner
        ...
        if (CommandCenter.falcon.nukeMeter > 0) list.add("PRESS N for NUKE")
